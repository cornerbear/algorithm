!function (t) {
    ...
    e.m = t, e.c = a, e.d = function (t, a, n) {
        ...
        if (e.r(n), Object.defineProperty(n, "default", {
            enumerable: !0,
            value: t
        }), 2 & a && "string" != typeof t) for (var s in t) e.d(n, s, function (a) {
            return t[a]
        }.bind(null, s));
        return n
    }, e.n = function (t) {
    ...
    var n = {
        ...
        navbars: [],
        showNavAdmin: !0,
        catalog: {enable: !1, index: !0, active: !1, levels: ["h2", "h3", "h4"]},
        signature: {
            enable: !0,
            author: null,
        ...
        sponsor: {enable: !1, text: "Buy me a cup of coffee ☕.", paypal: null, wechat: null, alipay: null}
    };
    ...
    e(2);
    var l = function () {
        const t = n.navbars, a = $("#navList");
        t && t.length && (t.reverse().forEach(t = > {if(t.target = t.target || "_self", t.chilren && t.chilren.length)
        {
            var e = t.chilren.map((function (t) {
                return t.target = t.target || "_self", `<li><a class="menu" target="${t.target}" href="${t.url}">${t.title}</a></li>`
            }));
        ...
        a.find("li").eq(1).after(`<li><a class="menu" target="${t.target}" href="${t.url}">${t.title}</a></li>`)
    }),
        ...
        $.each(a.children("li"), (t, a) = > {$(a).children("a").length ? $(a).append("<i></i>") : $(a).remove()}
    ),
    ...
    e(4);
    var c = function () {
        const t = n.avatar, a = n.favicon;
        t && $("#sideBarMain").prepend(`<img class="esa-profile-avatar" src="${t}" />`), a && $("#favicon").attr("href", a)
    };
    e(5);
    var r = function () {
        const t = n.catalog;
        if (t.enable) {
            const a = t.levels, e = a[0], n = a[1], s = a[2];
            let i = $("#cnblogs_post_body").find(a.join(","));
            if (!i.length) return !1;
            $("body").append(`<div class="esa-contents ${t.active ? "active" : "noactive"}"></div>`);
            let l = 0, o = 0, c = 0, r = "<ul>";
            $.each(i, (a, i) = > {const d = i.tagName.toLowerCase(), h = $(i).text();
            let u = $(i).attr("id"), p = "", f = h;
            if (t.index) d === e ? (l++, o = 0, c = 0, p = `<span class="level1">${l}. </span>`) : d === n ? (o++, c = 0, p = `<span class="level2">${l}.${o}. </span>`) : d === s && (c++, p = `<span class="level3">${l}.${o}.${c}. </span>`); else switch (d) {
                case e:
            ...
            u || (u = $(i).text().replace(/\ /g, "-").toLowerCase(), $(i).attr("id", u)), r += `<li class="${d}" title="${h}">\n                    <a class="esa-anchor-link" href="#${u}">${p + f}</a>\n                 </li>`, $(i).append(`<a href="#${u}" class="esa-anchor">#</a>`).hover(() = > {
                $(i)
                .find(".esa-anchor").css("opacity", 1)
            }, () =
        >
            {
    ...
    var u = function () {
        $("head").append(`<link rel="stylesheet" href="${d}"/>`), $.getScript(h, () = > {$("#cnblogs_post_body"
    ).
    ...
    var p = function () {
    ...
    )
        {
            var e = $(a).html();
            e = function (t) {
                var a = t.replace(/<span[^>]*>|<\/span>/g, "");
                /\r|\n$/.test(a) && (t += '<span class="ln-eof"></span>');
                var e = 1;
                return t = '<span class="ln-bg"></span>' + (t = '<span class="ln-num" data-num="1"></span>' + (t = t.replace(/\r\n|\r|\n/g, (function (t) {
                    return t + '<span class="ln-num" data-num="' + ++e + '"></span>'
                }))))
            }(e), $(a).html(e).addClass("hljsln");
            var n = $("span[data-num]:last");
            n.html() || n.remove()
        }
    })
    };
    ...
    var f = function () {
        const t = n.signature;
        if (t.enable) {
            const a = $("#cb_post_title_url").attr("href"),
    ...
    function v(t) {
        $("body").prepend(`<div class="esa-layer"><span class="content">${t}</span></div>`);
        let a = $(".esa-layer");
        a.slideDown(200), setTimeout(() = > {a.remove()}, 2e3
    )
    }
    ...
    var g = function () {
        ...
        var a = $(".esa-sponsor");
        a.find(".box li").hover((function () {
            const e = $(this).attr("class");
            if ("paypal" == e && !t.paypal) return v("未设置 PayPal 收款二维码");
            if ("alipay" == e && !t.alipay) return v("未设置支付宝收款二维码");
            if ("wechat" == e && !t.wechat) return v("未设置微信收款二维码");
            var n = null;
            switch (e) {
                case"paypal":
                    n = t.paypal;
            ...
            a.find(".qrshow").html(`<img src="${n}">`).show()
        }), (function () {
            a.find(".qrshow").hide()
        }))
    };
    e(11);
    var m = function () {
        const t = ".feedbackItem > .feedbackCon > .blog_comment_body", a = () =
    >
        {
            $(t).before("<div class='esa-comment-avatar'><a target='_blank'><img /></a></div>");
            let a = $(".feedbackItem > .feedbackCon");
            $.each(a, (function (t, a) {
                let e = null, n = $(a).find('span[id$="avatar"]');
                n.length && (e = $.trim($(n).text())), $(a).find(".esa-comment-avatar img").attr("src", e || "https://pic.cnblogs.com/face/sample_face.gif");
                const s = $(a).parent().find(".comment_date").next().attr("href");
                $(a).find(".esa-comment-avatar a").attr("href", $.trim(s))
            }))
        }
        ;
        if ($(t).length) a(); else {
            let e = 1, n = setInterval(() = > {$(t).length && (clearInterval(n), a()), 10 == e && clearInterval(n), e
            ++
        },
            500
        )
    ...
    var b = function () {
        ...
        const a = "silence-mode-" + currentBlogApp, e = "silence-theme-" + currentBlogApp, i = (new Date).getHours(),
            l = sessionStorage.getItem(e) || n.defaultTheme,
            o = sessionStorage.getItem(a) || ("auto" == n.defaultMode ? i >= 6 && i < 18 ? "light" : "dark" : n.defaultMode);
        ...
        })), c.find(".up").click(() = > {$("html, body"
    ).
        animate({scrollTop: 0}, 450)
    }),
        c.find(".mode").click(() = > {const t = "light" == $("html").attr("mode") ? "dark" : "light";
        sessionStorage.setItem(a, t), $("html").attr("mode", t)
    }),
        c.find(".skin").click(() = > {r.slideToggle()}
    ),
        r.find(".themes button").click((function () {
            const t = $(this).data("theme");
            sessionStorage.setItem(e, t), $("html").attr("theme", t)
        })), c.find(".contents").click(() = > {$(".esa-contents"
    ).
    ...
    e(13);
    const w = {show: () = > {$(".light-loading, .dark-loading"
).
    show()
},
    hide:() =
>
    ...
    new class {
        ...
        building() {
            l(), o(), i(), c(), b(), s() ? (r(), u(), p(), f(), g(), m()) : ($(".forFlow").css({marginLeft: "260px"}), $("#sideBar").show()), y.hide()
        }
    }
}]);